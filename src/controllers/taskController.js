/**
 * Task Controller — thin HTTP adapter
 *
 * Tasks don't have a dedicated SDK service. Complex queries
 * use the SDK connection for table prefixing.
 */
...
const list = async (req, res) => {
  ...
  let sql = `
    ...
    LEFT JOIN ${conn.table('task__cdata')} tc ON t.id = tc.task_id
    WHERE 1=1
  `;
  const params = [];
...
const get = async (req, res) => {
  ...
  const task = await conn.queryOne(`
    SELECT t.*,
           d.name as dept_name,
    ...
    LEFT JOIN ${conn.table('team')} tm ON t.team_id = tm.team_id
    LEFT JOIN ${conn.table('task__cdata')} tc ON t.id = tc.task_id
    LEFT JOIN ${conn.table('thread')} th ON th.object_id = t.id AND th.object_type = 'A'
    WHERE t.id = ?
  `, [id]);
  ...
  if (task.object_type === 'T' && task.object_id) {
    ticket = await conn.queryOne(`
      SELECT ticket_id, number FROM ${conn.table('ticket')} WHERE ticket_id = ?
    `, [task.object_id]);
  }
  ...
  res.json({
    ...
    data: {
      ...
      team: task.team_id ? { team_id: task.team_id, name: task.team_name } : null,
      ticket: ticket ? { ticket_id: ticket.ticket_id, number: ticket.number } : null,
...
const getThread = async (req, res) => {
  ...
  if (!thread) throw ApiError.notFound('Task not found');
  ...
  const total = parseInt(
    await conn.queryValue(`SELECT COUNT(*) FROM ${conn.table('thread_entry')} WHERE thread_id = ?`, [thread.id]) || 0, 10,
  );
